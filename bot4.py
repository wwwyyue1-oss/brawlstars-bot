...
# ─────────────────────────────────────────────
#  Налаштування (безпечне отримання токенів)
# ─────────────────────────────────────────────
TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN")
BRAWLSTARS_API_KEY = os.environ.get("BRAWLSTARS_API_KEY")

# Перевірка наявності токенів
if not TELEGRAM_TOKEN:
    print("❌ Помилка: TELEGRAM_TOKEN не знайдено в змінних середовища")
    print("💡 Використовуйте: docker run -e TELEGRAM_TOKEN=ваш_токен...")
    sys.exit(1)

if not BRAWLSTARS_API_KEY:
    print("❌ Помилка: BRAWLSTARS_API_KEY не знайдено в змінних середовища")
    print("💡 Використовуйте: docker run -e BRAWLSTARS_API_KEY=ваш_ключ...")
    sys.exit(1)
...
async def main():
    ...
    log.info(f"✅ Токен Telegram: {'✓' if TELEGRAM_TOKEN else '✗'}")
    log.info(f"✅ API ключ Brawl Stars: {'✓' if BRAWLSTARS_API_KEY else '✗'}")
