...
# Отримуємо токени зі змінних середовища
...
if not TELEGRAM_TOKEN:
    ...
    print("💡 Переконайтесь, що у файлі .env є рядок: TELEGRAM_TOKEN=ваш_токен")
    sys.exit(1)

if not BRAWLSTARS_API_KEY:
    print("❌ ПОМИЛКА: BRAWLSTARS_API_KEY не знайдено!")
    print("💡 Переконайтесь, що у файлі .env є рядок: BRAWLSTARS_API_KEY=ваш_ключ")
    sys.exit(1)
...
def format_player(p: dict) -> str:
    ...
    for i, b in enumerate(sorted_b[:5]):
        ...
        top_brawlers += f"  {emoji} {b['name'].title()} — 🏆 {b['trophies']} (макс: {b['highestTrophies']}) | ⚡ {b['power']} рівень\n"
